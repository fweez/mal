...
final class unitTests: XCTestCase {
    ...
    func testLiteralParsers() {
        XCTAssertEqual(tokenize("('foo)"), Result.success([Token.lparen, .tick, .symbol("foo"), .rparen]))
        XCTAssertEqual(tokenize("(~foo)"), Result.success([Token.lparen, .twiddle, .symbol("foo"), .rparen]))
        XCTAssertEqual(tokenize("(~@foo)"), Result.success([Token.lparen, .twiddleAt, .symbol("foo"), .rparen]))
        XCTAssertEqual(tokenize("(\"~@foo\")"), Result.success([Token.lparen, .string("~@foo"), .rparen]))
        XCTAssertEqual(tokenize("(\"'foo\")"), Result.success([Token.lparen, .string("'foo"), .rparen]))
    }
}
